...
static const qreal Q_PI = qreal(M_PI);

StyledDial::StyledDial(QWidget* parent)
    :QDial(parent)
{
    setStyle(new DialStyle);
...
void DialStyle::drawComplexControl(ComplexControl cc, const QStyleOptionComplex* opt_, QPainter* p, const QWidget* w) const
{
    if (cc == QStyle::CC_Dial) {
        if (const QStyleOptionSlider *opt = qstyleoption_cast<const QStyleOptionSlider *>(opt_))
        {
            ...
            if (opt->subControls & QStyle::SC_DialTickmarks) {
                //p->setPen(opt->palette.dark().color().darker(120));
                p->setPen(Qt::red);
                p->drawLines(calcLines(opt));
            }
            ...
            const qreal d_ = r / 6;
            const qreal dx = opt->rect.x() + d_ + (width - 2 * r) / 2 + 1;
            const qreal dy = opt->rect.y() + d_ + (height - 2 * r) / 2 + 1;
            ...
            QPointF dp = calcRadialPos(opt, qreal(0.70));
            ...
            //    p->drawLine(calcRadialPos(opt, qreal(0.90)), calcRadialPos(opt, qreal(0.96)));
            //}
...
void StyledDial::paintEvent(QPaintEvent* e)
{
    return __super::paintEvent(e);
}
